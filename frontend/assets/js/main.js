  ...
  fetch("/start-websocket-verification", {
    ...
    .then((data) => {
      if (data.success) {
        ...
        verificationMessage.textContent = "Vui lòng nhìn thẳng vào camera";
    ...
    .then((data) => {
      if (data.success && data.result) {
        ...
        if (isMatch && similarity > 0.8) {
          ...
          updateVerificationStatus("Đang xác thực...", "verifying", similarity);
            ...
            fetch("/verify-face-realtime", {
              ...
              .then((verifyData) => {
                if (verifyData.success) {
                  ...
                  updateVerificationStatus("Lỗi xác thực", "failed", 0);
                  verificationMessage.textContent =
  ...
  if (!idCardUrl) {
    ...
    fetch("/upload", {
      ...
      .then((data) => {
        ...
        if (data.success) {
          closeCameraModal();
          addMessage("Test camera thành công! Ảnh đã được upload.", "bot");
    ...
    .then((data) => {
      ...
      if (data.success) {
        ...
        fetch("/verify-face", {
          ...
          .then((verifyData) => {
            if (verifyData.success) {
              ...
              if (result.status === "success" && result.same_person === true) {
                ...
                addMessage("Xác thực khuôn mặt thất bại!", "bot");
                addMessage(formatVerifyResult(result), "bot");
              ...
              addMessage(
                "Xác thực khuôn mặt thất bại: " + verifyData.error,
                "bot"
              );
          ...
          .catch((error) => {
            addMessage("Lỗi khi xác thực khuôn mặt: " + error.message, "bot");
            idCardUrl = null;
          });
      } else {
        addMessage("Lỗi khi upload ảnh: " + data.error, "bot");
